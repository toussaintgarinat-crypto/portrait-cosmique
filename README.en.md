...
For local Python 3.11/3.12: install `requirements.txt`, then run `python scripts/installer_traduction.py`. The default model directory is `models/en-fr/`; set `PORTRAIT_TRANSLATION_DIR` at installation and runtime to override it. The official download is verified with SHA-256. OPUS-MT model by Jörg Tiedemann and Santhosh Thottingal, licensed CC-BY 4.0; its attribution is retained alongside the model.
