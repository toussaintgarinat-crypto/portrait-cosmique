...
COPY main.py llm.py traduction.py ./
COPY scripts/installer_traduction.py ./scripts/installer_traduction.py
COPY static/ ./static/

RUN useradd -m -u 1000 appuser && chown -R appuser:appuser /app
USER appuser
# Modèle livré avec l’image : aucun téléchargement ni clé à l’exécution.
RUN python scripts/installer_traduction.py
RUN python -c "from traduction import traduire_horoscope; assert traduire_horoscope('Hello world.')"
