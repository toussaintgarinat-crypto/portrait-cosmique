(function () {
  ...
  function rendreHolistique(d) {
    ...
    const nk=trad.nakshatra, ved=trad.vedique; if(nk||ved)racine.append(carteSimple(T().vedique,T().vediqueNote,[["Nakshatra",nk&&nk.nakshatra],[T().pada,nk&&nk.pada!=null?`${nk.pada} / 4`:null],[T().longitude,nk&&nk.longitude_siderale!=null?`${nk.longitude_siderale}°`:null],[T().rashi,ved&&traduireValeur(ved.rashi)],[T().ayanamsa,nk&&nk.ayanamsa!=null?`${nk.ayanamsa}° (Lahiri ≈)`:null],[T().precision,nk?tr(nk.precision,T().vediqueNote):null]],nk&&tr(nk.convention,T().vediqueNote),nk&&nk.nakshatra?`${nk.nakshatra} · ${T().pada} ${nk.pada}`:ved&&traduireValeur(ved.rashi),sens("nakshatra")||sens("vedique")));
    ...
    const cel=hol.celte_lunaire; if(cel)racine.append(carteSimple(T().celte,T().celteNote,[[tr("Arbre","Tree"),cel.arbre?tr(cel.arbre,cel.nom_en):T().intercalaire],[T().periode,langue()==="en" ? (["24 Dec–20 Jan","21 Jan–17 Feb","18 Feb–17 Mar","18 Mar–14 Apr","15 Apr–12 May","13 May–9 Jun","10 Jun–7 Jul","8 Jul–4 Aug","5 Aug–1 Sep","2–29 Sep","30 Sep–27 Oct","28 Oct–24 Nov","25 Nov–22 Dec"][cel.index-1] || T().intercalaire) : cel.periode],[T().cycle,cel.jour_cycle],[tr("Autre convention : 21 arbres (synthèse)","Other convention: 21 trees (synthesis)"),traduireValeur(trad.celte)]],tr(cel.convention,T().celteNote),tr(cel.arbre,cel.nom_en)||T().intercalaire,sens("celte")));
    const num=trad.numerologie_nom; if(num)racine.append(carteSimple(T().numero,T().numeroNote,[[T().expression,num.expression],[T().ame,num.ame],[T().personnalite,num.personnalite],[T().systeme,traduireValeur(num.systeme)]],"",`${T().expression} ${num.expression}`,sens("expression")));
  }
